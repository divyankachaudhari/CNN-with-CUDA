...
#define TILE_WIDTH 16 // Assuming a TILE_WIDTH x TILE_WIDTH block size for simplicity

__global__ void convMultiChannelKernelOptimized(
    const float* input, 
    const float* kernels, 
    float* output, 
    const float* biases,
    ...
    extern __shared__ float shmem[]; // Shared memory for input tile and kernel weights
    ...
    int z = blockIdx.z; // Output channel, assuming each block computes one output channel

    // Calculate the row and column index of the input
    ...
    for (int ch = 0; ch < numInputChannels; ++ch) {
        ...
        // Check if thread is within the input dimensions
        if ((row_i >= 0) && (row_i < inputHeight) && (col_i >= 0) && (col_i < inputWidth)) {
            shmem[ty * TILE_WIDTH + tx] = input[(ch * inputHeight + row_i) * inputWidth + col_i];
        } else {
            shmem[ty * TILE_WIDTH + tx] = 0.0f;
        }
        __syncthreads();
        ...
        if (ty < TILE_WIDTH && tx < TILE_WIDTH) {
            for (int i = 0; i < kernelSize; ++i) {
                for (int j = 0; j < kernelSize; ++j) {
                    int kernelIndex = (z * numInputChannels + ch) * kernelSize * kernelSize + i * kernelSize + j;
                    float weight = kernels[kernelIndex];
                    float value = shmem[(i + ty) * TILE_WIDTH + (j + tx)];
                    outputValue += weight * value;
                }
            }
        }
        __syncthreads();
    ...
    // Add bias and write the output
    if (row_o < inputHeight - kernelSize + 1 && col_o < inputWidth - kernelSize + 1 && z < numOutputChannels) {
        outputValue += biases[z];
        output[(z * (inputHeight - kernelSize + 1) + row_o) * (inputWidth - kernelSize + 1) + col_o] = outputValue;
    }
}
...
void runConvolutionAndAddBiases(const float* input, const float* kernels, float* output, 
    ...
    convMultiChannelKernel<<<numBlocks, threadsPerBlock>>>(d_input, d_kernels, d_output, d_biases, inputHeight, inputWidth, kernelSize, numInputChannels, numOutputChannels);
