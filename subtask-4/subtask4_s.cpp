...
// Updated to include a cudaStream_t parameter for stream-aware execution
void runConvolutionAndAddBiases(const float* input, const float* kernels, float* output, 
                    ...
                    cudaStream_t stream) { // Stream parameter added
    ...
    // Copy data to the device using cudaMemcpyAsync with the provided stream
    cudaMemcpyAsync(d_input, input, inputSize, cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(d_kernels, kernels, kernelSizeTotal, cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(d_biases, biases, numOutputChannels * sizeof(float), cudaMemcpyHostToDevice, stream);

    // Define grid and block dimensions
    dim3 threadsPerBlock(16, 16, 1);
    dim3 numBlocks((outputWidth + 15) / 16, (inputHeight - kernelSize + 1 + 15) / 16, numOutputChannels);

    // Launch the kernel using the provided stream
    convMultiChannelKernel<<<numBlocks, threadsPerBlock, 0, stream>>>(
        d_input, d_kernels, d_output, inputHeight, inputWidth, kernelSize, numInputChannels, numOutputChannels);
    
    addBiasesKernel<<<numBlocks, threadsPerBlock, 0, stream>>>(
        d_output, d_biases, outputWidth, inputHeight - kernelSize + 1, numOutputChannels);

    // Note: Synchronization is not performed here to allow concurrent execution; 
    // it should be managed outside this function if needed.

    // Free device memory - consider the lifecycle of your data; 
    // you might move these to the end of your stream processing
    cudaFree(d_input);
    cudaFree(d_kernels);
    cudaFree(d_output);
    cudaFree(d_biases);
...
int main() {
    ...
    loadWeightsAndBiases("trained_weights/conv1.txt", weightsConv1, biasesConv1, 20); // Assuming 
    ...
    loadWeightsAndBiases("trained_weights/fc2.txt", weightsFC2, biasesFC2, 10); // Assuming 10 biases for FC2 layer

    const int numStreams = 4; // Adjust based on the number of images you want to process concurrently
